# We modify the PSopt function from the NMOF library to include update a shiny progress bar
# How to update a function from package
...
# Shiny progress bar walkthrough
# https://shiny.rstudio.com/articles/progress.html

PSopt2 <- function(OF, algo = list(), updateProgress = NULL, ...) {
...
tmpfun <- get("PSopt", envir = asNamespace("NMOF"))
environment(PSopt2) <- environment(tmpfun)
attributes(PSopt2) <- attributes(tmpfun)
assignInNamespace("PSopt", PSopt2, ns = "NMOF")
